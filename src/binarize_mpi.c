...
int main(int argc, char* argv[]) {
    ...
    uint8_t *A_raw = NULL; // matrix A (only on master)
    uint8_t *T_raw = NULL; // matrix T (only on master)
    ...
    if (my_rank == 0) { /* MASTER */
        ...
        A_raw = malloc(n_size * n_size * sizeof(uint8_t));
        T_raw = malloc(n_size * n_size * sizeof(uint8_t));
    ...
    if (my_rows > 0) {
        my_A = malloc( sendcounts[my_rank] * sizeof(uint8_t));
        my_T = malloc( sendcounts[my_rank] * sizeof(uint8_t));
    }
    ...
    MPI_Scatterv(
        A_raw, sendcounts, senddispls, MPI_UINT8_T,  // send buffers
        my_A, (  sendcounts[my_rank] ), MPI_UINT8_T, // recv buffer
        0, MPI_COMM_WORLD
    );
    ...
    uint8_t *my_A_plus_ghosts = calloc(total_rows * n_size, sizeof(uint8_t));
    uint8_t *upper_ghost = my_A_plus_ghosts;                          // row -1
    uint8_t *local_data  = my_A_plus_ghosts + n_size;                 // rows [0 ... my_rows-1]
    uint8_t *lower_ghost = my_A_plus_ghosts + (my_rows + 1) * n_size; // row + my_rows

    /* copy local block into the central part of the buffer */
    if (my_rows > 0) {
        memcpy(local_data, my_A, my_rows * n_size * sizeof(uint8_t));
    }
    ...
    for (int i = 0; i < my_rows; i++) { // handles only valid rows
        for (int j = 0; j < n_size; j++) {
            ...
            uint16_t sum = 0; // sum of the neighborhood values
            uint8_t count = 0;    // number of elements, for the mean
    ...
    MPI_Gatherv(
        my_T, (my_rows * n_size), MPI_UINT8_T,      // send buffer
        T_raw, sendcounts, senddispls, MPI_UINT8_T, // recv buffers
        0, MPI_COMM_WORLD
    );
